...
class StaffProjectUsers(StaffProjectRoleMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        userroles = UserRole.objects.filter(project_id__in=[47, 183, 137, 105, 129], ended_at=None).exclude(user_id__in=current_users).distinct('user_id').values('id')
        context['available_user_list'] = UserRole.objects.filter(pk__in = userroles)
        return context
